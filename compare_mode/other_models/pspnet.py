...
from compare_mode.other_models import resnet
from torchvision import models
from compare_mode.other_models.base_model import BaseModel
from utils.helpers import initialize_weights, set_trainable
from itertools import chain

class _PSPModule(nn.Module):
    def __init__(self, n_channels, bin_sizes, norm_layer):
        super(_PSPModule, self).__init__()
        out_channels = n_channels // len(bin_sizes)
        self.stages = nn.ModuleList([self._make_stages(n_channels, out_channels, b_s, norm_layer)
                                                        for b_s in bin_sizes])
        self.bottleneck = nn.Sequential(
            nn.Conv2d(n_channels+(out_channels * len(bin_sizes)), out_channels,
    ...
    def _make_stages(self, n_channels, out_channels, bin_sz, norm_layer):
        prior = nn.AdaptiveAvgPool2d(output_size=bin_sz)
        conv = nn.Conv2d(n_channels, out_channels, kernel_size=1, bias=False)
...
class PSPNet(BaseModel):
    def __init__(self, n_channels, n_classes, backbone='resnet152', pretrained=True, use_aux=True, freeze_bn=False, freeze_backbone=False):
...
class PSPDenseNet(BaseModel):
    def __init__(self, n_channels, n_classes, backbone='densenet201', pretrained=True, use_aux=True, freeze_bn=False, **_):
